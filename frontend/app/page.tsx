    ...
    <div className="space-y-10">
      ...
      {!activeJobId && !discoverMutation.isPending && (
        <div className="grid grid-cols-3 gap-6 max-w-2xl mx-auto">
          {[
            { icon: Globe, label: "16 zemí", sub: "CZ, SK, DE, AT, PL, FR, IT, GB a další" },
            { icon: Store, label: "10 000+ obchodů", sub: "Od Alzy po malé e-shopy" },
            { icon: TrendingDown, label: "Reálné ceny", sub: "Živé scraping bez placených API" },
      ...
      {discoverMutation.isError && (
        <div className="max-w-2xl mx-auto p-4 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm">
          Nepodařilo se spustit vyhledávání. Zkontroluj zda běží backend.
        </div>
      )}
    </div>
  );
